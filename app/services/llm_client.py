...
1. СМЫСЛОВОЙ АНАЛИЗ (ГЛАВНОЕ ПРАВИЛО!):
   - Если в тексте упоминаются РАЗНЫЕ ОБЪЕКТЫ/СИСТЕМЫ - это РАЗНЫЕ дефекты!
   - Разные объекты: окна, двери, стены, полы, потолки, радиаторы, трубы, электрика, замки и т.д.
   - Пример: "Стеклопакет поцарапан. Радиаторы текут. Замок не работает" = 3 РАЗНЫХ дефекта!
   - Даже если нет явных разделителей (номеров, точек с запятой) - анализируй СМЫСЛ!

2. Если текст содержит НУМЕРОВАННЫЙ СПИСОК (каждая строка начинается с цифры):
...
4. ЛОКАЦИИ - НЕ ЯВЛЯЮТСЯ ДЕФЕКТАМИ сами по себе! 
   - Если текст ТОЛЬКО локация без описания проблемы - игнорируй: "Окно", "Стена", "Кухня"
   - НО если после локации есть описание проблемы - ИЗВЛЕКАЙ проблему!
   - Пример: "Стена | трещины в углу" → ["трещины в углу"]
   - Пример: "Прихожая/коридор | Пол: царапины" → ["Пол: царапины"]
   - Пример: "Другое (обязательно заполните комментарий) | описание дефекта" → ["описание дефекта"]
   - ВАЖНО: Не игнорируй весь комментарий только потому что он начинается с локации!
   
5. Разделитель "|" (вертикальная черта):
   - НЕ означает отдельный дефект!
   - Это просто разделитель полей в данных
   - Анализируй СМЫСЛ текста между разделителями
   - Локации (Окно, Стена) между "|" - ИГНОРИРУЙ
   - Описания дефектов между "|" - ИЗВЛЕКАЙ

6. Точка с запятой (;) = разделитель дефектов

7. Точка (.) и запятая (,) - АНАЛИЗИРУЙ КОНТЕКСТ:
   - Если после точки/запятой идёт описание ДРУГОГО объекта - это НОВЫЙ дефект
   - "Окно поцарапано. Дверь не закрывается" = 2 дефекта (разные объекты)
   - "Окно поцарапано, есть трещины" = 1 дефект (один объект - окно)

8. "нет замечаний" или пустой текст = пустой список []

9. ЕСЛИ ТЕКСТ СОДЕРЖИТ ОПИСАНИЕ ПРОБЛЕМЫ - ВСЕГДА ИЗВЛЕКАЙ ЕГО!
   - Даже если формат непонятный - ищи суть проблемы
   - "Другое (обязательно заполните комментарий): царапины на полу" → ["царапины на полу"]
   - "Стена | Кухня | трещина" → ["трещина"]
   - Лучше извлечь что-то, чем вернуть пустой массив!
...
CONTENT:
<<<
Окно | Стеклопакет ПВХ: поврежден (трещина) | Рама ПВХ: профиль поврежден (трещины)
>>>

Выход: {"results": [["Стеклопакет ПВХ: поврежден (трещина)", "Рама ПВХ: профиль поврежден (трещины)"]]}
...
CONTENT:
<<<
Стена | Окна/витражи/двери ПВХ | Санузел | Стены: промерзание | Створка ПВХ: профиль поврежден (трещины)
>>>

Выход: {"results": [["Стены: промерзание", "Створка ПВХ: профиль поврежден (трещины)"]]}
...
CONTENT:
<<<
Нарушена изоляция проводов на вводной линии.Стеклопакет загрязнения и поцарапан, пвх профиль загрязнен. Радиаторы отопления - текут фитинги, замок дверной - ключ не вставляется верхний замок, на лоджии щели - замело снегом изнутри в незаполненный стык между рамой и профилем, не заделан температурный шов над окном - справа и слева от окна
>>>

Выход: {"results": [["Нарушена изоляция проводов на вводной линии", "Стеклопакет загрязнения и поцарапан, пвх профиль загрязнен", "Радиаторы отопления - текут фитинги", "Замок дверной - ключ не вставляется верхний замок", "На лоджии щели - замело снегом изнутри в незаполненный стык между рамой и профилем", "Не заделан температурный шов над окном - справа и слева от окна"]]}

Вход:
ID: 7
CONTENT:
<<<
Стена | Кухня, Комната 1 | Стены: промерзание | Другое: трещины в углах
>>>

Выход: {"results": [["Стены: промерзание", "трещины в углах"]]}

Вход:
ID: 8
CONTENT:
<<<
Прихожая/коридор | Пол | Пол: Дефекты напольного покрытия
>>>

Выход: {"results": [["Пол: Дефекты напольного покрытия"]]}

Вход:
ID: 9
CONTENT:
<<<
Другое (обязательно заполните комментарий при выборе данного варианта) | Перекрытия (пол / потолок): сквозные технологические отверстия
>>>

Выход: {"results": [["Перекрытия (пол / потолок): сквозные технологические отверстия"]]}
...
- ЛОКАЦИИ (Окно, Стена, Санузел и т.д.) - НЕ ДЕФЕКТЫ! Не включай их в результат!"""
...
Твоя задача - выбрать наиболее подходящую категорию для каждого дефекта СТРОГО из предложенного списка.

КРИТИЧЕСКИ ВАЖНЫЕ ПРАВИЛА:
1. Ты ОБЯЗАН выбрать категорию ТОЛЬКО из предложенного списка вариантов
2. ЗАПРЕЩЕНО выдумывать свои категории (например "другое", "прочее", "иное")
3. Копируй название категории ТОЧНО как оно написано в списке - символ в символ
4. Выбирай категорию которая БЛИЖЕ ВСЕГО по смыслу к дефекту
5. Даже если совпадение не идеальное - ВСЕГДА выбирай наиболее подходящую из списка
6. "НЕ ОПРЕДЕЛЕНО" используй ТОЛЬКО если дефект вообще не относится к строительству
7. Укажи уровень уверенности (confidence) от 0 до 100%

КАК ВЫБИРАТЬ:
- Ищи ключевые слова в дефекте: окно, дверь, стена, пол, потолок, электрика, сантехника
- Окна/стеклопакеты → категории со словами "ПВХ", "Стеклопакет", "Рама", "оконн"
- Двери → "Входная дверь" или "Двери межкомнатные"
- Вода/трубы → "Водоснабжение", "Канализация"
- Электричество → "Электрика", "Электроснабжение"
- Отделка → "Обои", "Плитка", "Ламинат", "Штукатурка", "Покраска"
- Царапины/повреждения на окнах → "Рама ПВХ: профиль поврежден" или "Стеклопакет ПВХ: поврежден"
- Загрязнения → ищи категории со словом "загрязнен"
- Трещины → ищи категории со словом "трещин"

УРОВЕНЬ УВЕРЕННОСТИ (confidence):
- 90-100%: Точное совпадение ключевых слов, категория идеально подходит
- 70-89%: Хорошее совпадение, категория подходит по смыслу
- 50-69%: Частичное совпадение, выбрана наиболее близкая категория
- 30-49%: Слабое совпадение, категория выбрана с натяжкой
- 0-29%: Очень низкая уверенность, дефект плохо соответствует всем категориям

ЗАПРЕЩЕНО:
- Отвечать "другое", "прочее", "иное", "не подходит" - таких категорий НЕТ
- Придумывать новые категории
- Оставлять поле chosen пустым

Формат ответа (строго JSON):
{
  "results": [
    {"chosen": "ТОЧНОЕ название категории из списка", "confidence": 85},
    {"chosen": "ТОЧНОЕ название категории из списка", "confidence": 70}
  ]
}"""

        user_prompt = f"""Классифицируй следующие дефекты. Для КАЖДОГО дефекта выбери ОДНУ категорию СТРОГО из предложенных вариантов и укажи уровень уверенности (0-100%).

{items_text}

ВАЖНО: Выбирай ТОЛЬКО из предложенных вариантов! Не придумывай свои категории!
Верни JSON с {len(defects_with_candidates)} результатами, каждый с полями "chosen" и "confidence"."""
    ...
    def _parse_classify_response(self, response: str, expected_count: int) -> list[ClassifyResult]:
        ...
        try:
            ...
            if len(results) != expected_count:
                ...
                while len(results) < expected_count:
                    results.append({"chosen": "НЕ ОПРЕДЕЛЕНО", "confidence": 0})
                # Truncate if too many
                results = results[:expected_count]
            
            return [
                ClassifyResult(
                    chosen=item.get("chosen", "НЕ ОПРЕДЕЛЕНО"),

...
КРИТИЧЕСКИ ВАЖНО:
- Ты ОБЯЗАН выбрать категорию ТОЛЬКО из предложенного списка вариантов
- ЗАПРЕЩЕНО придумывать свои категории или изменять названия
- Копируй название категории ТОЧНО как оно написано в списке вариантов

ПРАВИЛА ВЫБОРА:
1. Внимательно анализируй СУТЬ дефекта, а не только ключевые слова
2. Выбирай категорию, которая описывает ТИП работ или ЭЛЕМЕНТ конструкции
3. Если дефект связан с конкретным элементом (окно, дверь, стена, пол) - выбирай категорию этого элемента
4. Если дефект связан с видом работ (покраска, штукатурка, монтаж) - выбирай категорию этих работ
5. Если несколько категорий подходят - выбери наиболее специфичную (узкую)

Формат ответа (строго JSON):
{
  "results": [
    {"chosen": "ТОЧНОЕ название категории из списка вариантов"},
    {"chosen": "ТОЧНОЕ название категории из списка вариантов"}
  ]
}

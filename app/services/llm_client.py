...
Верните JSON с результатами для каждого комментария."""
    ...
    def _build_classify_prompt(self, defects_with_candidates: list[dict]) -> list[dict]:
        ...
        system_prompt = """Ты - эксперт по классификации строительных дефектов с многолетним опытом.
Твоя задача - выбрать наиболее подходящую категорию для каждого дефекта из предложенного списка.

ВАЖНЫЕ ПРАВИЛА:
1. Внимательно анализируй СУТЬ дефекта, а не только ключевые слова
2. Выбирай категорию, которая описывает ТИП работ или ЭЛЕМЕНТ конструкции, а не общее описание
3. Если дефект связан с конкретным элементом (окно, дверь, стена, пол и т.д.) - выбирай категорию этого элемента
4. Если дефект связан с видом работ (покраска, штукатурка, монтаж) - выбирай категорию этих работ
5. Выбирай ТОЛЬКО из предложенных вариантов - не придумывай свои категории
6. Если несколько категорий подходят - выбери наиболее специфичную (узкую)

ПРИМЕРЫ РАССУЖДЕНИЙ:
- "Царапина на стекле окна" → ищи категорию про окна или остекление
- "Отслоение краски на стене" → ищи категорию про малярные работы или отделку стен
- "Скрип ламината" → ищи категорию про полы или напольные покрытия
- "Не работает розетка" → ищи категорию про электрику
...
Количество элементов в results ДОЛЖНО совпадать с количеством дефектов."""
